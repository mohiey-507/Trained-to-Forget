...
if __name__ == '__main__':
    ...
    if not os.path.exists(config.HAR_CSV_PATH):
        logging.error("Dataset not found. Skipping tuning.")
    else:
        study = optuna.create_study(direction="minimize")
        study.optimize(objective, n_trials=config.N_TRIALS)

        logging.info("--- Optuna Search Finished ---")
        logging.info(f"Best validation loss: {study.best_value:.5f}")
        logging.info(f"Best parameters: {study.best_trial.params}")
        
        with open(config.PARAMS_PATH, 'w') as f:
            json.dump(study.best_trial.params, f, indent=4)
        logging.info(f"Best parameters saved to {config.PARAMS_PATH}")
